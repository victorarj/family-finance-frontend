...
import { DocumentStatus, type Document } from "./documents.types";
import { useDocumentPolling } from "./useDocumentPolling";
import { formatDateTime, getDocumentStatusLabel, getSourceTypeLabel } from "../../utils/formatters";
  ...
  return (
    <Card className="space-y-3 border border-border/70">
      ...
      <div className="space-y-1 text-xs text-muted-foreground">
        <p>Enviado em {formatDateTime(document.uploaded_at)}</p>
        {processedAt && <p>Processado em {formatDateTime(processedAt)}</p>}
      </div>
    </Card>
  );
}
